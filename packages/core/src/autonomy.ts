...
// nodes with the fewest children (breadth), oldest first.
export async function selectNext(): Promise<string | null> {
  const res = await pool.query(
    `SELECT n.id
      ...
      WHERE n.state IN ('speculative','corroborating','corroborated')
        AND n.merged_into IS NULL
      ORDER BY n.is_launch_point DESC, COALESCE(ch.c, 0) ASC, n.created_at ASC
      LIMIT 1`
  );
...
export async function roamOnce(): Promise<RoamResult> {
  ...
  if (!id) return { status: "idle", expanded: 0, cost: 0 };
  const r = await expandForward(id, "erebus");
  if (r.blocked) return { status: "blocked", nodeId: id, expanded: 0, cost: r.cost, blocked: r.blocked };
  return { status: "expanded", nodeId: id, expanded: r.children.length, cost: r.cost };
}
